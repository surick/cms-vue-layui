...
<script>
    ...
    export default {
        data() {
            return {
                options: {
                    url: 'http://192.168.0.110:8989/cms/productType/getAllParentTypes?tokenAuthorization=' + localStorage.token,
                    cols: [[
        ...
        methods: {
            ...
            barEvent(obj) {
                ...
                if (obj.event === 'detail') {
                    ...
                    this.$layer.confirm('真的删除么', (index) => {
                        Category.deleteParentType(this, data.id).then((res) => {
                            if (res.success) {
                                obj.del();
                                this.$layer.msg('删除成功');
                    ...
                    Category.updateParentType(this, data.id, data).then(res => {
                        if (res.success) {
                            this.$layer.msg('修改成功');
            ...
            add() {
                this.$router.push('/product/category/parentAdd');
